...
Function UpdateMultDisk {
    ...
    $dateMs = ([TimeSpan] (Get-Date).ToShortTimeString()).TotalMilliseconds
    #$wc = New-Object System.Net.WebClient
    $urlFilesToDownload = @('https://github.com/marcusvd/PowerShell/raw/main/Backup/SingleDisk/BackupModule.psm1', 'https://raw.githubusercontent.com/marcusvd/PowerShell/main/Backup/MultiDisk/VMS.ps1')
    $pathToSaveFile = $env:HOMEPATH + '\Downloads'
    foreach ($url in $urlFilesToDownload) {
        Invoke-WebRequest -Uri $url -OutFile "$($pathToSaveFile)\$($url.split('/')[$url.split('/').Length - 1])"  -ErrorAction SilentlyContinue 
...
Export-ModuleMember -Function UpdateMultDisk
